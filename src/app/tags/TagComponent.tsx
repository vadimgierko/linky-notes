...
export default function TagComponent({ tag }: TagComponentProps) {
    ...
    return (
        <DropdownButton
            title={`${tag.tag} (${getTagNotesNum(tag.id)})`}
        ...
        >
            ...
            <Dropdown.Item
                onClick={async () => {
                    const agreed = confirm(`Are you sure you want to delete the tag ${tag.tag}?`);

                    if (!agreed) return;

                    if (tag.notes) return alert("Cannot delete tag with notes assigned... Remove the tag from it's notes, than try to delete the tag.");

                    await deleteTag(tag);
                    alert(`${tag.tag} tag was removed!`);
                }}
            >
                🗑️ delete tag
            </Dropdown.Item>
